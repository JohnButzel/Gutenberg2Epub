...
def create_output_directories(output_dir, author_name, book_title):
    output_directory = os.path.join(output_dir, author_name, book_title, "temp")
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)
    return output_directory

def download_css_files(css_links, url, output_directory):
...
def main():
    ...
    output_directory = create_output_directories(output_dir, author_name, book_title)
    css_links = soup.find_all('link', rel='stylesheet', href=True)
    download_css_files(css_links, url, output_directory)
